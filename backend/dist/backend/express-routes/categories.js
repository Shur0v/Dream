...
router.get('/', async (req, res) => {
    try {
        ...
        if (req.query.forceRefresh === 'true') {
            (0, db_1.invalidateCategoriesCache)();
        }
        const allCategories = await (0, db_1.getCategories)();
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const { id } = req.params;
        let category = await (0, db_1.getCategoryById)(id);
        if (!category) {
            category = await (0, db_1.getCategoryBySlug)(id);
        }
        if (!category) {
...
router.post('/', async (req, res) => {
    try {
        ...
        await (0, db_1.saveCategory)(newCategory);
...
router.put('/:id', async (req, res) => {
    try {
        await mockApiDelay(600);
        const { id } = req.params;
        const existingCategory = await (0, db_1.getCategoryById)(id);
        if (!existingCategory) {
            return res.status(404).json({
                success: false,
                error: 'Category not found',
        ...
        await (0, db_1.saveCategory)(updatedCategory);
...
router.delete('/:id', async (req, res) => {
    try {
        await mockApiDelay(600);
        const { id } = req.params;
        const deletedCategory = await (0, db_1.deleteCategory)(id);
        if (!deletedCategory) {
            return res.status(404).json({
                success: false,
                error: 'Category not found',
