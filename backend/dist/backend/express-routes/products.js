...
router.get('/', async (req, res) => {
    try {
        ...
        let filteredProducts = await (0, db_1.getProducts)();
        ...
        if (search) {
            ...
            filteredProducts = filteredProducts.filter(product => {
                ...
                if (product.tags && Array.isArray(product.tags)) {
                    if (product.tags.some(tag => tag && tag.toLowerCase().includes(searchLower)))
                        return true;
                }
                return false;
            });
        ...
        if (color) {
            filteredProducts = filteredProducts.filter(product => {
                if (product.colors && product.colors.length > 0) {
                    return product.colors.some(c => c.toLowerCase() === color.toLowerCase() || c === color);
                }
        ...
        // Filter by stock
        if (inStock) {
            filteredProducts = filteredProducts.filter(product => product.stock > 0 && product.isActive);
        }
        // Sort products
...
router.post('/', async (req, res) => {
    try {
        ...
        const { name, description, price, originalPrice, images, category, categoryId, subcategory, brand, sku, stock, colors, size, tags, specifications, } = req.body;
        ...
        if (colors && Array.isArray(colors) && colors.length > 0) {
            const allColors = await (0, db_1.getColors)();
            const validColors = colors.every(colorId => allColors.some(c => c.id === colorId));
            if (!validColors) {
                return res.status(400).json({
                    success: false,
        ...
        const newProduct = {
            ...
            sku: String(sku).trim(),
            stock: stockNum,
            colors: colors && Array.isArray(colors) && colors.length > 0 ? colors.filter((c) => c && c.trim() !== '') : [],
        ...
        await (0, db_1.saveProduct)(newProduct);
        res.status(201).json({
            success: true,
            data: newProduct,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const existingProduct = await (0, db_1.getProductById)(id);
        if (!existingProduct) {
            return res.status(404).json({
                success: false,
                error: 'Product not found',
        ...
        await (0, db_1.saveProduct)(updatedProduct);
        res.json({
            success: true,
            data: updatedProduct,
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const deletedProduct = await (0, db_1.deleteProduct)(id);
        if (!deletedProduct) {
