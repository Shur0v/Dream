 ...
 * 100% clone of BestSelling component structure
 * Shows products with matching related tags, or best selling products as fallback
 */
export default function RelatedProduct({ currentProduct }: RelatedProductProps) {
  ...
  useEffect(() => {
    const fetchRelatedProducts = async () => {
      try {
        ...
        // First, try to fetch products with matching tags
        ...
        if (currentProduct?.tags && currentProduct.tags.length > 0) {
          ...
          if (result.success && result.data) {
            // Filter products that have at least one matching tag and exclude current product
            relatedProducts = result.data.filter((p: Product) => 
        ...
        if (relatedProducts.length === 0) {
          ...
          if (bestSellingResult.success && bestSellingResult.data) {
            // Filter out current product and only show active products
            relatedProducts = bestSellingResult.data
              .filter((p: any) => {
                const productId = p.productId || p.id;
                return productId !== currentProduct?.id && p.isActive;
              })
              .slice(0, 4);
          }
        }
  ...
  return (
    <section className="father w-full py-6 sm:py-10 md:py-14 lg:py-20 bg-white flex flex-col justify-start items-center gap-2.5 sm:gap-5 md:gap-6 lg:gap-8" role="region" aria-labelledby="related-heading" data-layer="father">
